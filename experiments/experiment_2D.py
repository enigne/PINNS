...
def experiment_2D_3NN_test(weights, epochADAM=400000, epochLBFGS=0, N_u=2000, N_H=2000, N_f=4000, seed=1234, log_frequency=10000, history_frequency=10, NLayers=6, NNeurons=20,  #{{{
    ...
    #}}}
    # load the data {{{
    x, y, Exact_vx, Exact_vy, X_star, u_star, X_u_train, u_train, X_f, X_bc, u_bc, X_cf, n_cf, xub, xlb, uub, ulb, mu = prep_2D_data_all(path, N_f=hp["N_f"], N_u=hp["N_u"], N_s=hp["N_s"], N_H=hp["N_H"], N_C=hp["N_C"]) #}}}
    # Creating the model and training {{{
    logger = Logger(hp)
