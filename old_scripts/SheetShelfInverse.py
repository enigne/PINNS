...
class SSAInformedNN(NeuralNetwork): #{{{
    def __init__(self, hp, logger, X_f, 
        ...
        if FrictionCNN:
            ...
            self.trainableLayers = (self.model.layers[1:-1]) + (self.C_model.layers[1:-1])
...
# load the data
x, y, Exact_vx, Exact_vy, X_star, u_star, X_u_train, u_train, X_f, X_bc, u_bc, X_cf, n_cf, xub, xlb, uub, ulb = prep_Helheim_data(path, hp["N_u"], hp["N_f"], invertC=True)
